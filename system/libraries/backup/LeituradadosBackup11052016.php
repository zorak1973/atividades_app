...
class Leituradados{
				...
				if($linhas==2){
					switch($this->colunaSelecionada){
						...
						case('v'):
							...
							break;
						case('n_p_v_c'):
							$tabela .=  '<tr><td><b>Data/Hora</b></td><td><b>Chuva(mm)</b></td><td><b>Nivel(m)</b></td><td><b>Vaz&atilde;o(m3/s)</b></td><td><b>Cota</b></td></tr>';
							break;
			...
			if($linhas >= 3){
				...
				if((strtotime($dia) >= strtotime($this->inicio)) && (strtotime($dia) <= strtotime($this->fim)))
				{
					...
					switch($this->colunaSelecionada){
						...
						case('n_p_v_c'):
							$tabela .= "<td>".$valores2[$this->colChuva]."</td><td>".$valores2[$this->colNivel]."</td><td>".$valores2[$this->colVazao]."</td><td>&nbsp;</td>";					
						break;	
	...
	public function lerArquivoTxt(){
		...
		//LÊ UMA LINHA DO ARQUIVO
			$linhaAtual = fgets($this->ponteiro, 8192);
			if($linhas < 3){
				if($linhas==2){
					switch($this->colunaSelecionada){
						case('n'):
						...
						case('n_p_v_c'):
							$tabela =  "Data/Hora    Chuva(mm)      Nivel(m)    Vazão(m3/s)     Cota\r\n";
							fwrite($arquivoExportacao, $tabela);
							break;
			...
			if($linhas >= 3){
				...
				if((strtotime($dia) >= strtotime($this->inicio)) && (strtotime($dia) <= strtotime($this->fim)))
				{
					...
					switch($this->colunaSelecionada){
						...
						case('n_p_v_c'):
							$tabela = $dia2."\t".$teste2[$this->colChuva]."\t".$teste2[$this->colNivel]."\t".$teste2[$this->colVazao]."\r\n";				
							fwrite($arquivoExportacao, $tabela);	
						break;	
